-- handle EOF
module Parser (parse, preOrder, myShow) where
    ...
    myShow prog = 
        let 
            toks = alexScanTokens prog
            convert str = (splitSpace str)!!0 ++ " " ++ (splitSpace str)!!5 ++ ","
        in
            (init (foldl (++) "[" (map (\tok -> convert (show tok)) toks))) ++ "]"
    ...
    genError:: AlexPosn -> String

    genError pos = 
        case pos of
            AlexPn _ line column -> "Sytax Error:" ++ (show line) ++ ":" ++ (show column) ++ ":"
    ...
    preOrder expr =
        case expr of
            ...
            Const x -> [LeafBool x]
            Id s -> [LeafVar s]
            Bi opr expr1 expr2 -> [NodeBinOp opr] ++ (preOrder expr1) ++ (preOrder expr2)
            Uni opr expr1 -> [NodeUniOp opr] ++ (preOrder expr1)
    ...
    checkTok prev toks = 
        case prev of
            ...
            BIOPR -> if not (isBiOperator (head toks)) then True 
                    else error ((genError (token_posn (head toks))) ++ " expected expression after operator")
    ...
    parse_general toks = 
        ...
        in
            case tok of 
                ...
                PLUS p _ -> error ((genError p) ++ " Expected Expression before PLUS") 
                MINUS p _ -> error ((genError p) ++ " Expected Expression before MINUS") 
                TIMES p _ -> error ((genError p) ++ " Expected Expression before TIMES") 
                NEGATE p _ -> parse_normal_expr NONE [] [] toks
                EQUALS p _ -> error ((genError p) ++ " Expected Expression before EQUALS") 
                LESSTHAN p _ -> error ((genError p) ++ " Expected Expression before EQUALS") 
                GREATERTHAN p _ -> error (genError p)
                NOT p _ -> parse_normal_expr NONE [] [] toks
                AND p _ -> error ((genError p) ++ "Expected Expression before AND") 
                OR p _ -> error ((genError p) ++ "Expected Expression before OR") 
                XOR p _ -> error ((genError p) ++ "Expected Expression before XOR") 
                IMPLIES p _ -> error ((genError p) ++ "Expected Expression before IMPLIES") 
                CONST p _ -> parse_normal_expr NONE [] [] toks
                RPAREN p _ -> error ((genError p) ++ "Expected LPAREN before RPAREN") 
                IN p _ -> error ((genError p) ++ "Expected let before in") 
                THEN p _ -> error ((genError p) ++ "Expected if before then") 
                ELSE p _ -> error ((genError p) ++ "Expected if then before else") 
                ID p _ -> parse_normal_expr NONE [] [] toks
                ASSIGN p _ -> error ((genError p) ++ "Expected let id before =")
                FI p _ -> error ((genError p) ++ "Expected if then else before fi")
                END p _ -> error ((genError p) ++ "Expected let in before end")
    ...
    parse_paren toks = 
        if length toks < 3 then error (genError (token_posn (head toks)))
        else
            let 
                tok = nextToken toks
            in
                case tok of 
                    LPAREN p _ -> 
                        ...
                        in
                            if length toks_left < 1 then error ((genError p) ++ "No RPAREN found after LPAREN")
                            else
                                let 
                                ...
                                in
                                    case end_tok of
                                        ...
                                        _ -> error ((genError p) ++ "No RPAREN found after LPAREN")
                    _ -> error (genError (token_posn tok) ++ "No RPAREN found after LPAREN") 
    ...
    parse_LetIn toks = 
        case toks of
            (LET plet _):((ID _ var_name):((ASSIGN pasgn _ ):toks_left1))->
                if length toks_left1 < 3 then error ((genError pasgn) ++ " Invalid Let in expression")
                else 
                    let 
                        (expr_assign, toks_left2) = parse_general toks_left1
                    in
                        if length toks_left2 < 1 then error ((genError (getPos expr_assign)) ++ " Invalid Let in expression")
                        else if length toks_left2 < 2 then error ((genError (token_posn (head toks_left2))) ++ " Invalid Let in expression")
                        else case toks_left2 of 
                            (IN p _):toks_left3 -> 
                                let 
                                    (expr_in, toks_left4) = parse_general toks_left3
                                    binding = (var_name, (getExp expr_assign))
                                in
    ...
    parse_IfThenElse toks = 
        if length toks <  6 then error (genError (token_posn (head toks)))
        else case toks of
                ...
                if (length oprs == 0) then (op:oprs, exprs)
                    ...
                    in
                        if p1 > p2 then (op:oprs, exprs)
                            ...
                            case top_opr of
                                PosUni p opr ->
                                    if (length exprs) < 1 then error ((genError p) ++ " expected expression to match with uniary operator")
                                    else 
                                        let 
                                ...
                                PosBin p opr -> 
                                    if (length exprs) < 2 then error ((genError p) ++ " expected 2 expressions to match with binary operator")
                                    else 
                                        let 
    ...
    makeTree [] exprs = case exprs of 
                            ...
                            [expr] -> expr
                            _ -> error ((genError (getPos (last exprs))) ++ " incomplete expression")
    ...
    makeTree oprs exprs =
        ...
        in
            case opr of
                PosUni p op -> if (length exprs) < 1 then error ((genError p) ++ " expected expression to match with uniary operator")
                            else 
                                let
                                    expr = Create p (Uni op (getExp (head exprs)))
                                    in
                                        makeTree (tail oprs) (expr:(tail exprs))
                PosBin p op -> if (length exprs) < 2 then error ((genError p) ++ " expected expression to match with binary operator")
                            else
                                let
                                    expr1 = getExp (head exprs)
                                    expr2 = getExp (exprs!!1)
    ...
    parse_normal_expr prev oprs exprs toks = 
        let
            tok = nextToken toks
        in
            if (not (checkTok prev toks)) then error ((genError (token_posn (head toks))) ++ " expected operator after expression")
            else
                case tok of
                    PLUS p s -> 
                        let 
                    ...
                    ASSIGN p s -> error ((genError p) ++ "expected let id before =")
